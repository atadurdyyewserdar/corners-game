/**
 * Custom hook for managing game state with reducer pattern
 */
...
type GameAction =
  | { type: 'START_GAME'; cornerShape: CornerShape }
...
export function useGameState() {
  ...
  const startGame = useCallback((cornerShape: CornerShape) => {
    dispatch({ type: 'START_GAME', cornerShape });
  }, []);
